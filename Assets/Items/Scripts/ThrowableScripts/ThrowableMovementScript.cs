    ...
    private void OnCollisionEnter(Collision collision)
    {
        if (IsServer)
        {
            ...
            if (otherobject.GetComponent<PlayerHealthScript>() != null)
            {
                ...
                {
                    playerHealthScript.DamagePlayer(damage, properties.throwableType.ToString(), RandomMessages.Player_Death_Suicide(hitPlayerNickname));
                }
                else
                {
                    ...
                    switch (properties.throwableType)
                    {
                        case ThrowableType.snowball:
                            playerHealthScript.DamagePlayer(damage, "snowball", RandomMessages.Player_Death_Throwable_Snowball(hitPlayerNickname, properties.owner));
                            break;
